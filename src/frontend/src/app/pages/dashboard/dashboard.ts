...
export class Dashboard implements OnInit, OnDestroy {
  ...
  private loadRecentLogs(): void {
    ...
    this.logService.getRecentLogs(maxCount).subscribe({
      next: (recentLogs) => {
        const mappedLogs = recentLogs.map(l => ({ ...l, levelString: this.mapLevelToString(l.level) }));
        this.logsList.set(mappedLogs);
      },
  ...
  private listenToLiveLogs(): void {
    this.logService.liveLog$
      ...
      .subscribe((newLog) => {
        newLog.levelString = this.mapLevelToString(newLog.level);
        
        const currentLogs = this.logsList();
  ...
  protected mapLevelToString(level: number): string {
    switch (level) {
      case 0: return 'Info';
